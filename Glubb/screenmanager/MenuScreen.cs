...
namespace GameStateManagement
{
    ...
    abstract class MenuScreen : GameScreen
    {
            ...
            if (input.Gestures.Count > 0)
            {
                ...
                foreach (GestureSample gesture in input.Gestures)
                {
                    if (gesture.GestureType == GestureType.Tap)
                    {
                        ...
                        {
                            tapLocation = new Vector2(gesture.Position.X / (ScreenManager.Game as CatapultGame.TheGame).ScreenScale, gesture.Position.Y / (ScreenManager.Game as CatapultGame.TheGame).ScreenScale);
                        }
                        else
                        {
                            tapLocation = new Vector2(gesture.Position.X * ScreenManager.Scaler.Scale.X / (ScreenManager.Game as CatapultGame.TheGame).ScreenScale, gesture.Position.Y * ScreenManager.Scaler.Scale.Y / (ScreenManager.Game as CatapultGame.TheGame).ScreenScale);
                        }
        ...
        public override void HandleMouseInput(InputState input)
        {
            ...
            {
                Vector2 clickLocation = new Vector2(input.CurrentMouseState.Position.X * ScreenManager.Scaler.Scale.X / (ScreenManager.Game as CatapultGame.TheGame).ScreenScale, input.CurrentMouseState.Position.Y * ScreenManager.Scaler.Scale.Y / (ScreenManager.Game as CatapultGame.TheGame).ScreenScale);
